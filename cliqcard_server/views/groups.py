...
class GroupsView(MethodView):
    ...
    def get(self, group_id):
        if not group_id:
            ...
            group_member = GroupMember.query.filter_by(group_id=group.id, user_id=request.user.id).first()
            if not group_member:
                abort(401)
    ...
    def post(self):
        ...
        # create the group
        group = Group(name=name, admin_id=request.user.id)
    ...
    def get_join_code(group_id):
        ...
        if not join_code:
            ...
            join_code = GroupJoinCode(group_id=group.id, code=code, expiration=datetime.datetime.utcnow() + datetime.timedelta(days=14))
